...
export default class OpenAiHateThreateningTextSignal extends SignalBase<
  ScalarTypes['STRING'],
...
> {
  ...
  override get id() {
    return { type: SignalType.OPEN_AI_HATE_THREATENING_TEXT_MODEL };
  }

  override get displayName() {
    return 'OpenAI Hate + Threatening Text score';
  }

  override get description() {
    return `OpenAI's model that detects hateful threats, which is defined as hateful content that also includes violence or serious harm towards the targeted group.

      This model produces a confidence score between 0 and 1, indicating the model's confidence that the content is hateful and threatening. For example, if the model produces a score of 0.76, that means the model is 76% confident that the content in question contains hate and threats.`;
  }
