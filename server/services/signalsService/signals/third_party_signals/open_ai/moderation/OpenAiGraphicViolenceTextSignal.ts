...
export default class OpenAiGraphicViolenceTextSignal extends SignalBase<
  ScalarTypes['STRING'],
...
> {
  ...
  override get id() {
    return { type: SignalType.OPEN_AI_GRAPHIC_VIOLENCE_TEXT_MODEL };
  }

  override get displayName() {
    return 'OpenAI Graphic Violence Text score';
  }

  override get description() {
    return `OpenAI's model that detects graphic violence, which is defined as violent content that depicts death, violence, or serious physical injury in extreme graphic detail.

      This model produces a confidence score between 0 and 1, indicating the model's confidence that the content contains graphic violence. For example, if the model produces a score of 0.76, that means the model is 76% confident that the content in question contains graphic violence.`;
  }
